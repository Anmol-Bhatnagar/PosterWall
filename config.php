...
function generateHTMLPage(array $info, string $base64Image = '', string $mimeType = ''): ?string {
    ...
    $systemPrompt = 'You are a world-class web designer working for a company named "PosterWall.in" which is a product of "TechEagles" Under "MahaKubhrix Innovation" who creates STUNNING, PRODUCTION-READY HTML pages for Indian businesses. You write complete, beautiful, mobile-first HTML with embedded CSS and JS. Your designs are memorable, professional, and uniquely crafted for each business type. You NEVER use generic templates. You always return only valid HTML, no markdown, no explanation.';
...
MENU SECTION RULES — STRICTLY REQUIRED:
All menu categories (starters, mains, desserts, drinks, digestivo, gelato, etc.) MUST be rendered inside a SINGLE unified menu section. Do NOT create separate <section> blocks for individual menu categories. Do NOT create a special dark/colored section just for the last category of items. Every category — no matter how visually distinct — must live inside the same parent menu section. You may style individual categories differently (e.g. a dark subsection inside the menu), but they must all be children of one parent menu section. This is non-negotiable.

BRANDING FOOTER — STRICTLY REQUIRED:
As you work for PosterWall.in and this menu is made by posterwall.in, you MUST include this at the bottom of the page, styled as a natural part of the design:
...
Style it as centered text matching the page design — not inside a card, no box, no shadow, no border-radius. Subtly visible on all screen sizes and never cut off. This is non-negotiable. If you fail to include this correctly, the HTML will be rejected.
It should not hinder the actual menu items or services. It should be subtle — visitors should feel this is the business page, but subconsciously notice that PosterWall.in created it.
You MUST ensure this is visible on all screen sizes and is not cut off.

FOOTER PLACEMENT RULES — STRICTLY REQUIRED:
- Write the branding ONLY inside a <footer> tag.
- The <footer> tag must contain ONLY the branding text — no menu items, no category titles, no business info.
- Do NOT embed or nest the <footer> inside any <section>, <div>, or other wrapper element.
- The <footer> must be a direct child of <body> and must be the very LAST element before </body>.
- All business content (hero, menu, contact, timings, etc.) must be in <section> or <div> elements placed BEFORE the <footer>.
- Do NOT repeat the footer branding anywhere else on the page.
- Style it to match the business color scheme so it looks like a natural part of the page.

Correct structure (follow this exactly):
  <body>
    <div class=\"hero\">...</div>
    <section class=\"menu\"><!-- ALL menu categories go here, including digestivo/drinks --></section>
    <section class=\"contact\">...</section>
    <footer>
      <address>
        Created by <a href='https://posterwall.in' target='_blank' rel='noopener noreferrer'>PosterWall.in</a>
        <p>A Product of TechEagles | Under Mahakumbrix Innovation</p>
      </address>
    </footer>
  </body>

WRONG — never do this (common mistakes to avoid):
  MISTAKE 1: <section class=\"digestivo-section\">...items...<div>Created by PosterWall.in</div></section>
             Reason: branding is inside a section, not in a <footer> tag, and it corrupts the menu layout.
  MISTAKE 2: Creating a second top-level <section> just for the last menu category (like digestivo or drinks).
             Reason: all categories must be inside the single menu section.
  MISTAKE 3: Omitting the <footer> tag entirely and putting branding in a plain <div> at the bottom.
             Reason: a <footer> tag is required.
